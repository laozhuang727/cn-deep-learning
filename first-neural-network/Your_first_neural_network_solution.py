...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            output_error_term = error * 1  # output layer gradients

            # TODO: Calculate the hidden layer's contribution to the error
            hidden_error = np.dot(self.weights_hidden_to_output,
                                  output_error_term)  # errors propagated to the hidden layer

            # TODO: Backpropagated error terms - Replace these values with your calculations.
            hidden_error_term = hidden_error * hidden_outputs * (1 - hidden_outputs)  # hidden layer gradients

            # Weight step (input to hidden)
            delta_weights_i_h += hidden_error_term * X[:,None]  # ryan delta of the weights for the input->hidden
            # Weight step (hidden to output)
            delta_weights_h_o += output_error_term * hidden_outputs[:,None]  # ryan delta of the weights for the hidden->output
    ...
    def run(self, features):
        ...
        hidden_inputs = np.dot(features,
                               self.weights_input_to_hidden)  # signals into hidden layer # signals into hidden layer
        hidden_outputs = self.activation_function(hidden_inputs)  # signals from hidden layer
